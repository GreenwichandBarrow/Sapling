...
|---|--------|------|--------------------|
| 1 | Rule graduation | Rules Kay corrected 2+ times → stop hooks | `.codex/hooks/` + memory file |
| 2 | Memory consolidation | Stale/duplicated `memory/*.md` → merge or delete | `memory/` |
| 3 | Skill doc refresh | SKILL.md files with outdated column refs / superseded rules | `.agents/skills/*/SKILL.md` |
| 4 | Open-loop promotion | `brain/context/session-decisions-*.md` open loops → memories | `memory/` |
| 5 | **Skill learnings promotion** | Scan `.agents/skills/*/learnings.md`, invoke `evolve` per non-empty file | parent skill's SKILL.md / workflows / references |
...
**Process:**
1. `find .agents/skills -maxdepth 2 -name learnings.md -type f` — enumerate every skill's learnings inbox.
...
The agent drafts emails in Gmail (`gog gmail draft create`). Kay edits before sending. The edits are learning signal..
...
### Skill Usage Report
For every skill in `.agents/skills/` AND every launchd job:
...
Propose sunset as a standard calibration item. Kay approves the sunset → skill is moved to `.agents/skills/_archive/{date}-{skill}` with a commit message explaining why. Launchd job is disabled via `launchctl unload`.
