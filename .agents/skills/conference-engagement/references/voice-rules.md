...
Kay reviews each draft in Gmail Drafts, customizes the `{{callback}}` and any other per-recipient touches, and hits send. Codex drafts; Kay sends. Sending stays a Kay action — never auto-send from this skill.
