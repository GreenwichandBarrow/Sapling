...
- Presenting operator-decision gates **outside the digest file body**. The digest file's `## 5. Recommended Actions` section legitimately contains RECOMMEND/YES/NO/DISCUSS lines for Kay's later async review — that's expected output. The forbidden pattern is the Codex run itself emitting RECOMMEND→YES/NO/DISCUSS to the harness as if waiting for an answer (the 4/28 morning incident).
