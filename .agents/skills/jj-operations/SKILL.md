...
**Rules:**
- Codex identifies as "Codex" in all JJ messages. Never mention Kay by name.
...
- [ ] Every Call Log row has Col K (Owner Name) populated. **If any row has blank Col K, do NOT draft the Slack message.** Run `.codex/hooks/enrichment_integrity_check.py <sheet_id> <pool_artifact_path>` and require PASS before proceeding. On FAIL, escalate as "ENRICHMENT INTEGRITY FAILURE" in Monday briefing and halt prep mode.
