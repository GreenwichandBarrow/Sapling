# cold-call-operations — Headless Sunday Prep Run

You are running the legacy `jj-operations` skill as **cold-call-operations prep mode** non-interactively under systemd at Sunday 18:00 ET. There is no human in the loop. Do not ask clarifying questions, do not present YES/NO/DISCUSS gates, do not request approvals.

**Sheet header rule:** Do not hardcode column letters, column numbers, or fixed table ranges as business logic. Resolve fields by header name at runtime using `scripts/col-lookup.py` or an equivalent header-map read. If a required header cannot be resolved, stop and log the missing header.

## Mandatory ordering — execute in this exact sequence

1. **Read SKILL.md fully** at `.agents/skills/jj-operations/SKILL.md`.
...
3. **For each cold-call niche** (legacy `JJ-Call-Only`, default Premium Pest Management; multi-niche TBD):
   - Identify master sheet ID per `.agents/skills/jj-operations/SKILL.md` sheet mapping (Premium Pest = `1Y0ZjEkc2LHhBoO4QGO8Ny9MvG90NpojQn8bloKA291I`).
   ...
   - Copy the Full Target List rows that match pool artifact selection, resolving all fields by header name. **Drop rows where `Owner Name` is blank** from this week's pool (Apollo enrichment failures). If remaining pool drops below 100 rows after dropping blanks, STOP per Failure handling. Tier 1 only for Sunday-prep — do not mix in Tier 2 generic-gatekeeper rows even though SKILL.md allows them for pure cold calls.
4. **Cross-reference Do Not Call** per SKILL.md Step 2 — drop any company on DNC.
5. **Cross-reference skip flags** — drop rows where the first-call status header or any resolved skip/status field equals "PE-OWNED - SKIP" or any other skip flag.
6. **Cross-reference DealsX universe** per `memory/feedback_outreach_channel_universes_separate.md` (HARD rule, codified 2026-05-26) — drop any company that appears in the DealsX target sheet `1VaviHqaJT9Wtm6X1h9B6Q8aOrA8adTiBvt851pkEUFg` ("Greenwich & Barrow - New Verticals (May)"), Pest-relevant tabs:
   - `Specialty Pest & ENV Service (Good Fit) Valid` (`Company Name` header, ~55 contacts)
   ...
   **Tab-floor flag:** if dedup drops any daily tab below 20 rows (cold-call floor), emit `COLD-CALL WARN: {tab} below floor after DealsX dedup (N rows, floor 20)` to stdout. Do NOT auto-backfill — Kay decides whether to relax the filter or accept the short tab.
...
- Each tab has rows with `Owner Name` populated.
...
- **Pool artifact missing** → write `COLD-CALL STOP: pool artifact missing at brain/context/jj-week-pool-{date}.md` to stdout and exit normally. The wrapper-side validator (`scripts/validate_jj_operations_integrity.py`) will detect missing tabs and emit `VALIDATOR FAILED` to Slack.
- **Sheet write fails** → retry once, then STOP and exit normally. Validator catches missing tabs.
- **`Owner Name` blank for some pool rows** → drop the blank-owner rows (Apollo no-match) and proceed with the remaining Tier 1 pool. STOP only if remaining pool drops below 100 rows after blanks dropped. Validator accepts blank-owner-dropped pools as long as final tab count meets the floor.
- **`Owner Name` blank for ALL pool rows** → STOP — pool was selected but enrichment fully failed; do NOT create tabs. Validator catches this case (rows with `Company` set but `Owner Name` blank).

## Why this prompt exists

Bare legacy `/jj-operations` invocations without this prompt risk the silent-exit-0 failure mode. This prompt codifies the pre-flight gate that prevents the 4/19 incident's exact failure mode (tabs created on un-enriched pool).
