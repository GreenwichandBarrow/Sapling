...
- `preflight_failed` — true if Codex CLI 401
...
**Bridge audit preservation.** Health-monitor RED bridge execution must leave an audit trail. As of 2026-08-07, the bridge is Slack-disabled by default per Kay preference; health-monitor findings belong in the health report, dashboard, and Good Morning, not urgent Slack. `HEALTH_MONITOR_SLACK_BRIDGE=1` is required to re-enable bridge-triggered launchd-debugger spawns. The daily headless prompt still preserves existing same-day `results[]` entries instead of overwriting them with a zero-failure daily scan.

## v1.2.0 — 2026-05-06

**Health-monitor RED bridge.** Legacy/opt-in trigger path: `scripts/health-monitor-red-bridge.sh` reads the weekly health-monitor markdown artifact at `brain/trackers/health/{TODAY}-health.md` and, only when `HEALTH_MONITOR_SLACK_BRIDGE=1`, fans out one `launchd-debugger:on-failure` spawn per RED row in the standard health tables. Yellow rows stay informational. The Trend table is filtered (parser keys on `NF==5` standard rows + `$3 == "RED"` status column; Trend rows have `NF==6` with RED in historical columns). Default behavior is dashboard/Good Morning only.
