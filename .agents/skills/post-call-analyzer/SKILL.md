...
5. Staged task handoff guard: if `brain/trackers/post-call-analyzer/pending-tasks/*.json` contains one or more non-empty task files older than the next Good Morning cycle after `staged_at` / file mtime, validator emits `HANDOFF-WARN: staged post-call tasks pending Good Morning review`. Legacy backlog is a Good Morning/System Health action item, not a hard post-call-analyzer failure, because the daily scan may have succeeded. Zero-task files may remain for audit only if they are explicitly marked zero-action or archived.
...
Validator failure → wrapper overrides exit code → Slack alert with `VALIDATOR FAILED` prefix per `feedback_mutating_skill_hardening_pattern.md`. Handoff warnings do not override the post-call-analyzer exit code; they must be consumed by Good Morning and System Health until Kay approves or rejects the staged tasks.
