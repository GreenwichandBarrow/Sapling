...
- **`To Do` tab** — the single capture point for all tasks. Columns: **A=Status, B=Task, C=Type, D=Project, E=Due, F=Notes, G=Horizon, H=Day of the Week**.
  - **`Status`** = native dropdown: `Not Completed` / `On-going` / `Completed` / `Dropped`. Replaces the old native checkbox + the archive-todo sweep. **"Done" = `Status == "Completed"`; "Dropped" = consciously removed/no longer pursuing.** Done-row conditional formatting fires on `Status == "Completed"`.
...
- **7 day tabs** (`Sun Mon Tue Wed Thu Fri Sat`, immediately after `Week`) — the calm, large-font daily *execution* surface. Kay works these Mon–Sat. Per-day layout follows the 6.7 reference: row 1 title, row 4 `HABITS` plus `SUPPLEMENTAL`, rows 5–14 primary habits (A/B), supplemental habits (C/D), secondary supplemental habits or goal (E/F), row 16 `DAILY FOCUS`, row 17 focus checkbox + value, row 20 task headers, rows 21–70 fifty priority slots (A=native checkbox · B=Task 17pt · C=Type dropdown · D=Project dropdown · E=Notes), rows 21–23 shaded sage as the top-three priority band, row 71 `NOTES`, rows 72–79 free-notes block. Builder: `scripts/build_day_tabs.py` (idempotent; expands grid; `--dry-run`).
...
5. Copy the reconciled prior `To Do` backend into the NEW file's `To Do` tab, with `On-going` rows first, then `Not Completed`, then `Completed` and `Dropped` rows sorted/moved to the bottom.
...
**To Do backend sort order:** Sunday build and cleanup pack rows in this order: `On-going` first, `Not Completed` second, `Completed` then `Dropped` last. This keeps recurring/permanent operating items visible while still keeping completed rows at the bottom for review.

**Cleanliness model:** No checkbox-sweep, no donut/%-display, and no separate `Completed To Do` tab. Completed rows stay in the `To Do` tab with `Status=Completed` and render via done-row CF; Dropped rows stay in the `To Do` tab with `Status=Dropped` and represent consciously removed/no-longer-pursuing work. For Kay's Sunday review, both should sort/move to the bottom of the `To Do` list. `On-going` rows stay at the top, followed by `Not Completed`; `Completed` and `Dropped` rows stay packed at the bottom. Empty/gap rows ARE physically removed — see compaction doctrine below.
...
**Recurring priority doctrine (codified 2026-07-20 per Kay):** recurring means baseline obligation, not top priority. When `schedule-from-todo-days` populates the Week planning tab, explicit `Day of the Week` assignments should fill each day first; recurring `Horizon = Weekly Recurring {day}` items append afterward unless Kay manually elevates them. The green top-three band is a judgment band for the week's highest-value work, especially active sprint/deck/analysis work, not a reserved slot for recurring/admin tasks. Recurring/ongoing items must not be placed in the top-three rows, even if they also have a Day of the Week value; if a day has fewer than three non-ongoing assigned priority items, leave the remaining top-three slots blank and start recurring/ongoing items below the band.

**Recurring schedule scaffold doctrine (codified 2026-08-07 per Kay):** recurring rows may define the weekly rhythm as broad working blocks, not one-off task content. Monday-Thursday scaffold blocks are Outreach Work, Relationship / Call Window, Strategy / Research, and 8:30 Decision Review. Friday scaffold blocks are Personal Finance / Wealth Monitor, Nonprofit Operations, Family Admin, Systems / Website / G&B Housekeeping, and 8:30 Decision Review. Treat these as containers/prompts; slot actual tasks underneath them on the Week tab. Do not add `Review Good Morning Brief` as a recurring To Do item unless Kay explicitly asks for it; it is the daily operating trigger, not a default checkbox. `Weekly Review + Next Week Setup` should become a Friday recurring scaffold only once the post-9/10 Friday schedule starts.
...
**Prior-day cleanup after carry-forward (codified 2026-06-18 per Kay; overflow fix 2026-06-20; duplicate guard 2026-07-14):** when `carry-forward-day` moves unchecked items from a prior day into the current day during Good Morning or Good Night, it must scan every task row above the visible `NOTES` header, not just the canonical rows 17-66. If manual edits push tasks below the 50-slot block, those overflow rows still carry forward. Before writing into the destination day, compare normalized task text against existing destination tasks; if the task already exists there, treat it as already carried forward, clear the source row, and do not create a duplicate. If the destination day lacks enough blank task rows, insert overflow task rows immediately above `NOTES` and immediately re-apply native checkbox validation, Type/Project dropdowns, 17pt task text formatting, and task-row height to every inserted overflow row. After moving, clean the source day tab: checked/completed task rows pack to the top, any remaining unchecked rows follow, and blank rows are pushed below. This keeps the prior day readable after moved items are cleared and prevents black/blank gaps from accumulating.
...
**Missed-bookend Good Night repair (codified 2026-06-27 per Kay):** Good Night should normally carry the closeout date into the next operating day. If Kay explicitly says the prior Good Night and same-day Good Morning were both missed, first check the report for stranded incomplete rows on earlier live day tabs and repair those rows into the next operating day with explicit `carry-forward-day --from {prior_day} --to {next_day}` dry-run + execute. This prevents Friday work from staying stranded when the next actual bookend is Saturday night.
...
Populate the Week planning tab from the `To Do` tab column H `Day of the Week`. Use after Kay reviews/cleans the backend and assigns days. Completed, Dropped, and blank-day rows are skipped; task rows are written packed to the top of each Week day block. Daily tabs are not populated until Kay approves the Week plan and asks to distribute.
