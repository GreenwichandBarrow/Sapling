...
Standing owner of mechanical sheet maintenance across the G&B operational stack. Codifies the Codex-Kay contract defined in `memory/feedback_tracker_manager_scope.md`.
...
| Master Call List (JJ's sheet) | (various tabs) | Partial — JJ owns; Codex only writes via jj-operations skill workflow |
...
- Column conventions: `.agents/skills/niche-intelligence/references/tracker-access.md`

## Evolution

This skill is expected to grow. When Codex surfaces-for-approval on something repetitive with a consistent Kay-YES pattern, graduate it to auto-execute (update this SKILL.md and the memory rule). When an auto-action causes a problem, demote it to surface-for-approval. Revisit scope at each quarterly investor-update cycle.
