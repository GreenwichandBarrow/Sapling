...
5. **Step 2 — IDENTIFY + INDUSTRY VALIDATION (sequential)**: run `niche-intel-identifier` to surface 1-5 new niche candidates with INITIAL SCREEN + TARGET TAM + MARKET TAM blocks per SKILL.md. Niche count of 0 is acceptable only if the synthesizer convergence report is empty — document the reason in the final report.
   - **Kay-directed override (MANDATORY — 2026-06-04 calibration):** A niche Kay explicitly named or queued (tagged Kay-directed in `brain/inbox/`, or named in session-decisions) MUST reach WEEKLY REVIEW even when it hits a soft-exclude (construction-adjacent, B2C, travel-intensive). Do NOT silent-drop it. Advance the row with the exclude attached as a visible flag (`SOFT-EXCLUDE: {reason} — Kay-directed, surfaced for analyst review`) so the analyst sees both the directive and the concern. Soft-excludes are reflexes for *agent-discovered* niches, not vetoes on a CEO directive. A hard regulatory/legal kill-reason (e.g. VA-benefits compliance) still kills, but keep the kill-reason on the row. See `feedback_kay_directed_niche_overrides_exclude`.
   - **Disposition discipline:** niche status moves (table/kill/advance) require an explicit operational instruction, not a conversational lean. Never table/kill a niche off a casual chat remark. See `feedback_dont_execute_decisions_from_chat`.
