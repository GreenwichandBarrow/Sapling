...
- `--slot` is 1-15 (rows 14-28 on the target day tab)
...
Run AFTER Kay finalizes the week on the Week tab. Reads each Week-grid day-block's 15 priority slots (status + task) + habit checkboxes and writes them into the corresponding day tab's slots (rows 14–28) + habits (rows 4–12).
...
- `--day` accepts Sun..Sat / Mon..Sun (resolves to the matching day TAB)
- `--slot` is 1..15 (rows 14-28) — **optional**; if omitted, auto-picks the first empty slot for that day tab. (Single-step alternative to `append` → `promote`.)
...
- `--slot` is optional; omit for auto-pick at stamp time. Numeric 1..15. (Stored in Notes as a slot pin if given.)
...
4. **Trace decision-content writes** to `brain/traces/{date}-task-tracker-{verb}-{slug}.md` with what changed + snapshot path. Trace emission applies ONLY to `move-day-item`, `promote`, `projects-create-gantt`, `sync-done-status`, `recurring-add`, and `recurring-remove` verbs — those carry a choice between alternatives. **Mechanical-receipt verbs do NOT trace** — `build-week` (+ its `archive` alias), `distribute-week`, `schedule-to-day-slot`, `reformat`, and `compact-todo` route their rollback receipt to `brain/context/verb-logs/{date}-task-tracker.log` instead (via `log_verb_receipt()`), and `append` routes to `logs/scheduled/task-tracker-{date}.log`. Rationale: these are receipts of an instruction Kay already gave (put task in slot N, apply CF rules, Sunday rollover), not decisions, and they pollute calibration input. Source: 2026-06-04 calibration — 12 of 38 traces (32%) in the prior batch were these receipts (and 2026-05-08 — 6 of 35 were `append`). The rollback record is preserved in verb-logs, just out of `brain/traces/` (decision-traces SKILL.md anti-pattern #6). **`sync-done-status` is no-op-aware:** it writes a trace ONLY when ≥1 To Do row actually flipped — no-op runs leave no trace. **`recurring-add` / `recurring-remove` always trace** because each edit compounds across every future Sunday rollover.
...
- Trace files are mandatory for `promote`, `move-day-item`, `projects-create-gantt`, `recurring-add`, and `recurring-remove` (decision-content verbs). `sync-done-status` writes a trace conditionally (only when ≥1 row was actually flipped). Mechanical-receipt verbs (`build-week`/`archive`, `distribute-week`, `schedule-to-day-slot`, `reformat`, `compact-todo`) route to `brain/context/verb-logs/{date}-task-tracker.log`, and `append` to `logs/scheduled/task-tracker-{date}.log` — receipts are not decisions and pollute calibration input (2026-06-04 calibration). `gantt-tick` and `report` traces remain optional.
