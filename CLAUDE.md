...
| `niche-intelligence` | Tuesday 22:30 ET | Newsletter scrape, niche identification, one-pagers, scorecards. **Staggered 2026-04-29 from 23:00 → 22:30** to break parallel-fire collision with `nightly-tracker-audit` after both failed simultaneously on 4/28 with `error: An unknown error occurred (Unexpected)` (3 attempts each, exit 1). Bead `ai-ops-5wx` queues hardening to mutating-skill standard (headless prompt + validator + wrapper case + plist `:tuesday` mode arg) — until then this fire still falls through to bare `/niche-intelligence` and is expected to fail next Tuesday too. |
...
- Apply both to any **mutating** scheduled skill (writes to sheets / Attio / vault). Read-only skills (deal-aggregator, email-intelligence) tolerate silent-0 because absence-of-output surfaces in pipeline-manager. **All 5 mutating scheduled skills now wired (2026-04-26, beads `ai-ops-jrj` + `.1`-`.4`):** `target-discovery` Phase 2, `jj-operations-sunday`, `nightly-tracker-audit`, `weekly-tracker`, `relationship-manager`. Each has its own `scripts/validate_{skill}_integrity.py` validator + `headless-{mode}-prompt.md` headless prompt. Wrapper case-statement in `scripts/run-skill.sh` routes `skill:mode` args to the corresponding prompt file.
