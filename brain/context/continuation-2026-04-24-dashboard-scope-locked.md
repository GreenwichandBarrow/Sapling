...
| 3 | **Active Deal Pipeline** | NDA-forward conversations only (renamed from "Deal Pipeline" 2026-04-24). 4-column Kanban + post-NDA closed strip. Featured as hero tile on Dashboard landing. | Daily glance |
| 4 | **M&A Analytics** | Activity rollups — daily / weekly / quarterly / LTD. **Replaces Weekly Activity Tracker Google Sheet** once weekly-view ships. Hosts outbound funnel stats (Identified + Contacted counts, DealsX/Kay/JJ split, reply rates, conversion to NDA) — confirmed by Kay 2026-04-24 PM as the right home for funnel data, NOT a separate landing tile. Funnel stats are one section of the broader M&A retrospective; landing surfaces the conversion outcome (NDA-forward) instead of raw outreach volume. | Weekly for core metrics |
