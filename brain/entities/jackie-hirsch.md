---
schema_version: 1.1.0
date: 2026-07-07
type: person
status: prospect
company: "[[entities/crowne-atlantic]]"
role: Business broker / M&A advisor
email: jackie@crowneatlantic.com
attio_id: 9d80670f-c573-495b-8ead-f3d7088eb59d
attio_url: https://app.attio.com/greenwich-barrow/person/9d80670f-c573-495b-8ead-f3d7088eb59d
attio_synced_at: 2026-07-09T10:54:32Z
tags:
  - date/2026-07-07
  - entity
  - person
  - person/jackie-hirsch
  - company/crowne-atlantic
---

# Jackie Hirsch

## Quick Facts
- Warm intro target via [[entities/beth-odonnell|Beth O'Donnell]].
- Email address captured from the intro thread: `jackie@crowneatlantic.com`.

## Communication Style
- **Tone:** Not yet established
- **Format:** Not yet established
- **Frequency:** Ad hoc, based on intro thread

## Key Context
- Introduced in a thread titled "Quick Introduction" on 2026-07-06.
- Thread suggests Kay should respond directly and keep the connection warm.

## Relationship Notes
- 2026-07-07: Created from email-intelligence intake after the intro thread surfaced.
- 2026-07-09: Backfilled Attio record `9d80670f-c573-495b-8ead-f3d7088eb59d` during Attio-vault alignment repair.
