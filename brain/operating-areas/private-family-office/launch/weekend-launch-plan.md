# Weekend Launch Plan

Date: September 11, 2026

## Objective

Launch a small, credible paid-product test for The Private Family Office: one useful digital-first household document system that a stranger can understand, purchase, and receive automatically.

This is an income test, not proof of salary replacement. The goal is to collect evidence: visits, email signups, paid purchases, refunds, and buyer questions.

## Reusable Work

- Brand positioning: **A private family office system for the modern home.**
- Homepage visual direction: eight flat books, 4 across by 2 rows on desktop, grey and peach neutral palette.
- First live book: **Risk & Protection**.
- First envelope structure: confidential tie-envelope interface with a reading panel.
- First free sample: **Identity Documents Checklist**.
- First paid-product direction: **TPFO Household Protection Binder**.
- Initial discovery channel: Pinterest as visual search, supported by SEO on the owned site.
- Product recommendation standard: recommend only products Kay has personally used, tested, or directly vetted.
- Personally vetted product: Savor Folio Document Organizer.

## Missing For A Paid Launch

- Final product name.
- Final product scope and exact included pages.
- Final price. **Set for first test: $49.**
- Payment and delivery platform. **Selected for first test: Gumroad direct checkout.**
- Email capture platform or form destination.
- Final product file in PDF format.
- Product preview images.
- Public sales page.
- Public free-download page or form.
- Terms, privacy, refund language, and affiliate disclosure.
- Search-agreement or conflict review before commercial launch.
- Approved distribution assets.

## Smallest Credible Weekend Scope

The smallest credible test is not the full eight-book site. It is:

- A simple homepage or product landing page using approved Private Family Office styling.
- A Risk & Protection product page for **TPFO Household Protection Binder**.
- A free **Identity Documents Checklist** for email capture.
- One paid PDF product with a clean preview and automatic delivery.
- A small number of Pinterest assets pointing to the free checklist and paid product page.
- Basic measurement through platform analytics and checkout reporting.

## What The Agent Can Execute

- Draft the paid-product outline.
- Draft sales-page copy.
- Draft free-download page copy.
- Draft Etsy listing copy as an optional near-term marketplace test.
- Draft Pinterest pin concepts and SEO keywords.
- Build local static HTML prototypes.
- Create editable source files for product pages and downloads.
- Prepare launch checklist and platform comparison.

## What Requires Kay

- Approve final product name.
- Approve exact product contents and disclaimers.
- Approve any change to the $49 first test price.
- Choose payment/delivery platform.
- Create or authorize platform accounts.
- Approve any public website publication.
- Approve any paid advertising or marketplace listing fee.
- Approve search-agreement/conflict review before commercial launch.
- Confirm whether any product language relies on personal experience.

## Post-Launch Testing

After launch weekend, consider testing alternate product names only if there is enough traffic to make the results useful. Keep **TPFO Household Protection Binder** as the first launch name.

## Recommended Weekend Sequence

1. Finalize product scope and price.
2. Finalize the free Identity Documents Checklist.
3. Finish the paid TPFO Household Protection Binder PDF.
4. Build a simple sales page and email capture page.
5. Choose payment and delivery platform.
6. Connect checkout and delivery after approval.
7. Publish after approval.
8. Launch the first discovery test through Pinterest and, if approved, Etsy.

## Customer Acquisition Test

The near-term problem is not just production. The test needs traffic from people who are already searching for a solution.

### Owned Site + Pinterest

Pinterest fits the brand because it acts more like visual search than personality-led social media. It is slower than a marketplace but can compound over time.

Initial Pinterest asset themes:

- Important Documents Checklist
- TPFO Household Protection Binder
- Family Office System for the Home
- Identity Documents Checklist
- Organizing Important Documents
- Home Document Organizer
- Modern Household Binder

### Etsy Marketplace Test

Etsy is not the long-term brand home, but it may provide faster search-intent exposure for terms such as emergency binder, important documents binder, family documents organizer, and just-in-case binder.

The brand risk is that Etsy's visual environment is busy and often crisis-framed. The test listing should keep Private Family Office language calm and differentiated while using marketplace search terms where necessary.

Potential test cost: listing fees and platform transaction/payment fees. Kay must approve before listing.

## Commercial Evidence To Track

- Page visits.
- Email signups.
- Free checklist downloads.
- Paid product page views.
- Add-to-cart or checkout starts if available.
- Purchases.
- Refund requests.
- Net proceeds after platform fees.
- Buyer questions or confusion.
- Search terms or pin/listing analytics.

## Launch Readiness Questions

- What is the exact paid product name?
- What is the price for the first test?
- Should the first test run on the owned site only, Etsy only, or both?
- Which platform should collect email addresses?
- Which platform should process payment and deliver the PDF?
- What refund language feels appropriate?
- Should a legal/compliance review happen before taking payments?

