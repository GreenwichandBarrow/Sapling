---
schema_version: "1.2.0"
date: 2026-05-13
type: budget-report
status: published
skill_origin: budget-manager
kay_approved: null
kay_approval_date: null
people:
  - "[[entities/anthony-bacagan]]"
tags:
  - date/2026-05-13
  - output
  - output/budget-report
  - status/published
  - topic/budget
  - topic/runway
  - topic/fund-management
  - source/quickbooks
  - person/anthony-bacagan
---

# Budget Report: April 2026

Monthly budget report for April 2026. Data sourced from [[entities/anthony-bacagan|Anthony Bacagan]] (StartVirtual) April 2026 Management Report delivered 2026-05-12 (QBO accrual basis). Auto-fired by email-intelligence per `feedback_bookkeeper_pl_auto_trigger_budget_manager.md` — first deterministic auto-trigger cycle since trigger chain was repaired.

## Headline Numbers

- **April Net Burn:** $33,296.47 (vs March $31,903.60 — $1,393 higher MoM)
- **Cash on hand (Apr 30):** $190,523.61 (Checking $3,601.06 + Savings $186,922.55)
- **Available for Operations:** $150,523.61 (after $40K DD reserve)
- **Forward Steady-State Burn (net of interest):** ~$21,050/mo
- **Runway from May 1:** 7.1 months → projected zero **Dec 2026**
- **Shortfall vs Feb 2027 deadline:** **-2 months** (improved from March's -3.5)
- **Monthly savings needed:** **$4,325/mo** (improved from March's $7,393)

## Material Change vs Prior Month

**Health insurance fully paid.** April absorbed the final $8,250 installment. Annual budget $33,000 vs YTD actual $33,536.91 — annual plan effectively complete. **Forward months (May onward) drop health to $0**, materially extending runway. This confirms the 2026-05-11 March-report REFRAME CORRECTION (health is one-time front-load, not recurring), now validated by April actuals.

**Runway improved by 2.5 months** (March -3.5 mo shortfall → April -2 mo shortfall). Monthly savings need cut nearly in half ($7,393 → $4,325). Wed/Fri financial work (desk sublet, Mercury yield, Gusto renegotiation, software stack cuts) remains buffer/optionality, but the cushion is real.

## Fund Position (as of April 30, 2026)

| Metric | Value |
|--------|-------|
| Fund Balance (cash) | $190,523.61 |
| Checking 4538 | $3,601.06 |
| Savings 8125 | $186,922.55 |
| Remaining Health Installment | $0 (annual plan paid in full) |
| Available for Operations | $150,523.61 (after $40K DD reserve) |
| DD Reserve (held back) | $40,000 |
| Forward Steady-State Burn (gross) | ~$21,600 |
| Interest Income (monthly avg) | $548 |
| Net Steady-State Burn | ~$21,052 |
| Runway from May 1 (steady-state) | 7.1 months |
| Projected Zero Date | Dec 2026 |
| Fund Deadline | Feb 2027 |
| **Shortfall vs Deadline** | **-2 months** |
| Monthly Savings Needed | $4,325 |
| Total Invested | $551,825.01 |
| Budget % Remaining | 34.5% |

## Monthly P&L Summary

| Category | Jan 2026 | Feb 2026 | Mar 2026 | Apr 2026 | YTD |
|----------|----------|----------|----------|----------|-----|
| Regular Wages | $12,692 | $12,692 | $15,865 | $12,692 | $53,942 |
| Payroll Taxes | $1,533 | $1,172 | $1,214 | $971 | $4,890 |
| Health & Accident Plans | $8,787 | $8,250 | $8,250 | $8,250 | $33,537 |
| Business Insurance | $0 | $55 | $48 | $37 | $140 |
| Contractors - VA (JJ) | $1,537 | $394 | $1,040 | $1,287 | $4,258 |
| Contractors - Analyst | $387 | $490 | $317 | $0 | $1,193 |
| Office Rent | $1,000 | $1,000 | $1,000 | $2,000 | $5,000 |
| Office Supplies | $220 | -$9 | $46 | $94 | $350 |
| Postage & Shipping | $0 | $1 | $26 | $681 | $708 |
| Apps & Software - CRM | $220 | $318 | $1,345 | $73 | $1,956 |
| Apps & Software - Office Use | $227 | $585 | $1,224 | $465 | $2,500 |
| Databases & Research | $1,962 | $282 | -$1,238 | $1,451 | $2,457 |
| Advertising & Marketing | -$116 | $247 | $244 | $2,422 | $2,798 |
| Professional Fees (Accounting) | $0 | $3,200 | $0 | $0 | $3,200 |
| Professional Fees (Consulting) | $0 | $0 | $911 | $0 | $911 |
| Business Taxes & Licenses (NEW) | $0 | $0 | $0 | $1,322 | $1,322 |
| Memberships & Subscriptions (NEW) | $0 | $0 | $0 | $260 | $260 |
| Travel - Car/Shared Rides | $429 | $178 | $819 | $511 | $1,937 |
| Travel - Meals & Entertainment | $1,049 | $1,720 | $1,330 | $891 | $4,991 |
| Travel - Airfare | $2,028 | $0 | $0 | $348 | $2,376 |
| Travel - Vehicle Gas | $0 | $48 | $0 | $0 | $48 |
| Bank Fees | $4 | $5 | $6 | $0 | $15 |
| **Total Expenses** | **$31,958** | **$30,628** | **$32,447** | **$33,755** | **$128,789** |
| Interest Income | $654 | $532 | $544 | $459 | $2,189 |
| **Net Burn** | **$31,304** | **$30,096** | **$31,904** | **$33,296** | **$126,600** |

## Variance Flags (>10% over YTD prorated budget at end of Apr / 4 mo elapsed)

YTD prorated budget = Annual budget / 12 × 4 months elapsed.

1. **Advertising & Marketing: $2,798 YTD vs $333 prorated (+740%)** — HYPOTHESIS LOGGED 2026-05-13
   April $2,422 single-month spike. Hypothesis (CEO): DealsX (cold-email lead-gen) + AI consultant fees were re-categorized into this bucket. Math is plausible: prior Feb/Mar ran ~$245/mo (steady-state ad subscription); April adds ~$2,180 above run-rate, consistent with DealsX subscription + a consultant invoice. **Quick bucket-tracking confirm needed from Anthony** — want to know going forward whether DealsX + AI consultant continue to land here vs Professional Fees (Consulting). Note: Professional Fees (Consulting) shows $911 in March only, $0 in April — supports the re-categorization theory.

2. **Travel - Meals & Entertainment: $4,991 YTD vs $1,668 prorated (+199%)**
   Still over but slowing. April $891 was the lowest month YTD. Trend is decelerating — may settle within tolerance by year-end. $5K annual budget already nearly exhausted at 4 mo; budget revision likely needed.

3. **Travel - Car/Shared Rides: $1,937 YTD vs $668 prorated (+190%)**
   April $511 — second-highest month. Conference travel continues. Annual budget $2,000 will exhaust in May at this pace.

4. **Office Rent: $5,000 YTD vs $4,000 prorated (+25%)** — RESOLVED 2026-05-13
   April hit $2,000 (double the $1,000/mo run-rate). Confirmed: early May rent prepay; rate is still $1,000/mo. May actual should be $0; YTD normalizes back to budget by end of May. No Anthony question needed.

5. **Postage & Shipping: $708 YTD vs $17 prorated (+4136%)**
   April $681 spike. $50 annual budget is meaningless against actuals. Likely conference materials shipping or one-time send. Watch May.

6. **Apps & Software - Office Use: $2,500 YTD vs $1,200 prorated (+108%)**
   YTD continues over budget but April $465 was modest. March $1,224 spike (annual renewal) already absorbed.

7. **Health & Accident: $33,537 YTD vs $11,000 prorated (+205%)**
   Timing-driven — annual plan paid in full as of April. **No further charges expected for 2026.** This converts from variance flag to "annual line item closed." Will normalize off the YTD prorated comparison in coming months.

8. **Accounting/CPA: $3,200 YTD vs $1,068 prorated (+200%)**
   Annual CPA filing one-time hit (Feb). Closed.

9. **Travel - Airfare: $2,376 YTD vs $1,000 prorated (+138%)**
   Jan front-load ($2,028) + April $348. Below $250/mo budget for Feb/Mar/Apr — should normalize.

10. **Business Taxes & Licenses: $1,322 YTD (NEW LINE — not in approved budget)** — RESOLVED 2026-05-13
    First-time line item in April. CEO confirmed: annual state filing (Delaware franchise / NY state filing). One-time hit; should not recur until next year's filing cycle. No Anthony question needed.

11. **Memberships & Subscriptions: $260 YTD (NEW LINE — not in approved budget)** — RESOLVED 2026-05-13
    First-time line in April. CEO confirmed: tech-stack subscription (vendor TBD but bucket is correct). Folds into ongoing tech-audit work to identify the $4,325/mo savings target. No Anthony question needed.

12. **Consulting Fees (Professional Fees subcat): $911 YTD vs $0 budgeted (NEW)**
    Carryover from March. Still need to confirm what this covers — flagged in March report, still open.

**Net favorable items:**
- Databases & Research: $2,457 YTD vs $5,000 prorated (49% under).
- Regular Wages + Payroll Taxes: $58,832 YTD vs $58,333 prorated (essentially on-budget, +0.9%).
- Analyst contractor: $1,193 YTD vs $1,200 prorated (on-budget).
- Bookkeeper line: $0 booked YTD vs $988 prorated. Anthony's invoices still haven't hit QBO (carried from March). **Surface to Anthony.**

## Runway Analysis

**Prior estimate (March report, original calc):** 7.5 months from April 1, projected zero Nov 2026, **3.5 month shortfall**.
**Prior estimate (March REFRAME correction):** 9.46 months on existing cash, projected zero mid-Feb 2027, no shortfall.

**Current estimate (April actuals):** 7.1 months from May 1, projected zero Dec 2026, **2 month shortfall**.

The April actual confirms the REFRAME thesis directionally — health installments stopped, forward burn drops — but the runway lands between the original and reframe estimates. Why:

1. **Forward steady-state burn is higher than the optimistic REFRAME** ($21,050 vs March's reframe of an implied $19,000). New recurring/lumpy items not in original budget added to the run-rate: business taxes ($110/mo amortized), memberships ($22/mo), advertising spike (TBD if recurs), April postage spike. (April $2,000 rent confirmed as May prepay — not a structural addition.)
2. **Cash on hand is lower than projected** ($190,524 vs $223,820 March end) because April burn ($33,296) was elevated by the final $8,250 health installment plus the $2,422 ad spend, $1,322 business taxes, and $681 postage — all material one-offs concentrated in a single month.

Net: ~2 months better than March's original projection, ~3 months worse than the March REFRAME's most-optimistic scenario. Realistic baseline.

**Scenarios:**
- **Status quo ($21,050/mo net):** Zero Dec 2026, miss deadline by ~2 months.
- **Cut $4,325/mo (target):** Hit Feb 2027 deadline exactly. Tech audit + bookkeeper transition + advertising discipline.
- **Cut $2,000/mo:** Zero Jan 2027, ~1 month shortfall.

## Action Items

1. ~~Confirm April $2,000 rent with Anthony.~~ RESOLVED 2026-05-13 — early May rent prepay. May actual will be $0; YTD normalizes by month-end.
2. **Confirm April Advertising bucket placement with Anthony.** Hypothesis: DealsX + AI consultant landed in Advertising. Want to lock the bucket-tracking convention going forward.
3. ~~Confirm Business Taxes & Licenses ($1,322).~~ RESOLVED 2026-05-13 — annual state filing (Delaware / NY). One-time.
4. ~~Confirm Memberships & Subscriptions ($260).~~ RESOLVED 2026-05-13 — tech-stack subscription. Folds into tech audit.
5. **Follow up with Anthony on bookkeeper line posting.** Still $0 booked YTD — invoices haven't hit QBO since March surfacing. Reconcile billing.
6. **Run `/budget tech-audit`** — Apps & Software Office Use still trending over. Tech audit may identify the $4,325/mo savings target.
7. **Watch Travel - Meals & Entertainment.** $5K annual budget on track to bust by Q2-end. Either accept reality and revise budget, or impose discipline.

## Outcome

- **Published:** 2026-05-13 (auto-fired by email-intelligence per repaired trigger chain).
- **Engagement:** Sheets updated (Tab 1 April column K + Apr +/- column L + bottom block; Tab 2 fund position + Apr Net Burn inserted row + runway block + investor reporting block, "As of April 30, 2026" date-stamped). Vault artifact created.
- **Material findings:** Runway improved 2.5 months vs March original (-3.5 → -2 shortfall); monthly savings need cut from $7,393 → $4,325. Health installment cycle complete — forward burn drops materially. Three new line items appeared in April (Business Taxes, Memberships, ad spike) requiring Anthony follow-up.
