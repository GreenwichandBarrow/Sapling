...
- `post-call-analyzer` reviewed: faithful plus improved. It now preserves transcript capture, Google Drive analysis artifacts, Attio notes, Good Morning approval routing for action items, 1Password/Granola access, MCP-free execution, and the no-direct-TO-DO/no-email-send safety contract.
- `conference-discovery` reviewed: faithful plus improved. It keeps the Google Sheet as the main deliverable, posts a tight Slack notice with sheet link and counts, preserves future/current `Skip` rows on the Pipeline until their dates pass, and keeps divider/formatting protection explicit.
- `deal-aggregator` reviewed: faithful plus improved. The active prompts preserve full listing review logs, morning/afternoon/Friday cadence, draft-only/no-send rules, 1Password access, and the Phase 2.5 search calibration around recurring/reoccurring revenue, cohort/customer durability, service criticality, $750K lower bound, $3M+ preference, and retail/restaurant hard no.
- `target-discovery` and cold-call operations reviewed: mostly faithful plus improved. The Sunday sequence is now target-discovery at 3pm ET and cold-call operations at 6pm ET; target-discovery owns enrichment/pool artifacts and cold-call operations owns Mon-Fri call tabs. Updated prompt language to use systemd rather than launchd and current cold-call naming while keeping legacy `JJ-*` identifiers only where validators/sheets still require them.
- `niche-intelligence` reviewed: faithful plus improved. It preserves the Tuesday night analyst-prep cadence, Identify -> One-Pager -> Score -> Tracker flow, organic niche intake path, header-based tracker access, 1Password-first credentials, read-only/no-send Gmail usage, and fallback behavior when optional MCPs are unavailable.
- `run-agent-skill.sh` reviewed: faithful plus improved. It exports `TODAY`, loads 1Password/token environment before job-specific env, enforces the global Codex scheduled-jobs kill switch, performs email safety preflight, maps scheduled job ids to headless prompts, runs Codex headlessly, and posts Slack failures when the operations webhook is available.
- `email-intelligence` reviewed: faithful plus improved. It preserves weekday 7am execution, compact Gmail context generation, read-only/no-send Gmail safety, Granola ingestion, CIM/NDA/bookkeeper/active-deal deterministic auto-triggers, raw-email log hygiene, required artifact schema, and explicit non-zero handling for auth failures.
- `relationship-manager` reviewed: faithful plus improved. It preserves daily relationship-health output, Attio REST-first access, Gmail evidence reads with `--gmail-no-send`, action-already-taken verification, cadence-field-as-source-of-truth rules, vault-to-Attio sync, warm-intro tracking, graceful degradation, and required artifact validation.
- `health-monitor` reviewed: faithful plus improved. It preserves the four-agent health architecture, scheduled-job coverage audit, consecutive-failure escalation, service REST checks before RED statuses, dashboard freshness checks, pipeline hygiene checks, and Slack escalation for RED findings.
- `launchd-debugger` reviewed: faithful plus improved despite legacy name. It now runs as a systemd-era repair layer with daily and on-failure prompts, strict FIX/SURFACE rules, known-incident and cross-day Slack suppression, no business-data writes, and REST verification before calling MCP disconnects real outages. Rename is Phase 3 cleanup, not a Phase 2 blocker.
- `weekly-tracker` reviewed: mostly faithful plus improved. It preserves Friday metric collection, six collector agents, sheet/vault/Drive snapshots, Slack notification, 1Password credentials, no-MCP dependency, and validator backstop. Updated the headless prompt to say Codex/systemd instead of launchd.
- `nightly-tracker-audit` reviewed: gap found and repaired. It still had column-letter/range-based tracker logic; updated the skill and prompt to resolve all required fields by header name before moving Tabled/Killed niches, resorting, and renumbering.
- Continue final scheduled/core pass for any lower-tier on-demand skills surfaced by hook/runtime scan.
