...
|-------|----------|---------|
| `deal-aggregator` | Mon-Fri 7:30am ET | Single morning platform scan + email screening after the 7am email-intelligence artifact lands; duplicate afternoon and Friday digest timers retired 2026-06-19 to prevent dashboard/status mismatches |
| `email-intelligence` | Mon-Fri 7am ET | Gmail/Granola scanning, email-scan-results artifact |
| Cold Call Operations (`jj-operations` legacy id, prep) | Sunday 6pm ET | Creates Mon-Fri Call Log tabs for the week. Reads pool artifact from **today's** target-discovery Phase 2 run (3pm fire). Kay reviews tabs Sunday evening before bed. |
| Cold Call Operations (`jj-operations` legacy id, harvest) | Manual | Read Call Logs and update the master sheet. Triggered by orchestrator or manually after the cold-call block ends. |
...
| `target-discovery` (Phase 2) | Sunday 3pm ET | Weekly owner enrichment via Apollo + web research on Cold-Call-Only target sheets. **Moved from 10pm → 3pm 2026-04-26** to ensure pool artifact exists before Cold Call Operations' 6pm fire (was a logical-ordering bug — prep can't read a pool that hasn't been written yet). 3-hour buffer accommodates enrichment + validator + retry; validator failures hit Slack with enough lead time for Kay to react before 6pm. **Hardened 2026-04-25:** wrapper passes `phase2-sunday` arg → headless prompt loads instead of bare `/target-discovery`; POST_RUN_CHECK runs `scripts/validate_phase2_integrity.py` per active Cold-Call-Only niche; validator failure overrides exit code → Slack alert. Bead `ai-ops-1`. **Scoped to pool-only 2026-05-31:** the validator passes `--pool-only` to `enrichment_integrity_check.py` — it checks every pool row has the Owner Name field populated, but does NOT walk the Mon-Fri Call Log tabs. Those tabs don't exist until Cold Call Operations creates them at 6pm, so the old full-mode check produced ~192 false "missing tab" failures every Sunday. Tab existence + Owner Name population are covered by `validate_jj_operations_integrity.py` at 6pm + Monday briefing warn-only. |
...
| `launchd-debugger` | Daily 8:20am ET + on-failure | Runs after 7am email-intelligence and 7:30am deal-aggregator as the final pre-brief double-check; also auto-fires on non-zero scheduled-skill exits. Hardened with POST_RUN_CHECK validator. |
