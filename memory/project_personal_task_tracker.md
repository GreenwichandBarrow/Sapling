...
**Week-tab layout** (modelled on the verbatim `archive_May 11-17` grid copy but RE-ORDERED Sun→Sat; the archive grid was Mon-first): col 0 = habit/notes label; for day i (0=Sun..6=Sat) status checkbox col = `1+2*i`, content/task col = `2+2*i`. Row 1 merged A1:O1 title `WEEK OF May 17-23` (16pt bold, sage-light) · row 5 `HABIT TRACKER` · row 6 Sun..Sat 2-col-merged sub-headers · rows 7–16 ten primary habit rows (label col 0, native checkbox per day) · row 17 SUNDAY..SATURDAY 2-col-merged day headers (11pt bold, white/sage-dark, carry the date) · rows 25–49 twenty-five priority slots/day (status checkbox col + task col, wrap, done-row CF), with each day block's first three slots shaded sage · row 51 notes sub-headers · rows 52–59 merged free-notes block/day. Native checkboxes, sage palette. **No donut** (donuts removed entirely 2026-05-17).

**Per-day-tab layout:** row 1 merged title `SUNDAY · May 17` (20pt bold, sage-dark) · row 4 `HABITS` plus `SUPPLEMENTAL` · rows 5–14 ten primary habit rows (A/B checkbox+label) plus ten supplemental habit rows (C/D checkbox+label; row 10 can carry `GOAL`) · row 16 column headers (✓|Task|Type|Project|Notes, 12pt) · rows 17–41 twenty-five priority slots (A=native checkbox · B=Task **17pt** · C=Type dropdown · D=Project dropdown · E=Notes; ~34px), with rows 17–19 shaded sage as the top-three priority band · row 43 `NOTES` · rows 44–51 free-notes block. **Donut: each day tab DOES retain its native pieChart donut(s) (anchored col G) — the 2026-05-17 "removed" claim was never executed; the 7 charts + helper tab `_retired__donut_data_2026-05-17` (gid 1387061206) were only hidden/renamed and still render. As of 2026-05-18 the donut counts HABIT completion per day** (`=COUNTIF({Day}!A5:A14,TRUE)` done / `=COUNTA({Day}!B5:B14)-COUNTIF(...)` left) — repointed from the prior to-do priority-slot binding per Kay. 2 duplicate charts per day tab (pre-existing; not deduped).
