...
- **Blank weekly template:** `G&B TO DO Weekly Template - Codex Reference 2026-06-14` (`1EaznKNTweSVRxbXpoEA2CyXLD8P96mWVU0K38-5pMxc`) is the canonical structural source for new weekly files. It should remain empty: structure, formatting, formulas, validations, habit/task frames, notes frames, and project scaffolding only.
- **Each Sunday's `build-week`:** resolves the prior file → copies the blank weekly template into `STRATEGIC PLANNING` as the new week → reconciles prior daily tabs into the prior `To Do` backend (checked exact matches mark `Completed`; missing day-tab tasks are added; conservative consolidated daily task edits update backend task shape) → snapshots prior state → copies reconciled `To Do` backend into the new file with `On-going` rows first, then `Not Completed`, then `Completed` and `Dropped` rows at the bottom → removes template placeholder/example project tabs such as `PROJECT 1`, then copies prior project tracking/real project tabs into the new file after template-owned tabs → copies prior daily tabs into far-right archive tabs in the new file → populates recurring rows and explicit day assignments onto Week as a baseline → validates → Kay reviews `To Do`, marks known completions, schedules active items onto Week, and approves Week → `distribute-week` flows approved Week to daily tabs → moves prior file into `To Do Archive` → updates pointer atomically as last step
...
1. **`To Do` tab** — the single capture point. Columns: **A=Status, B=Task, C=Type, D=Project, E=Due, F=Notes, G=Horizon, H=Day of the Week**.
   - **`Status`** = native dropdown: `Not Completed` / `On-going` / `Completed` / `Dropped`. Replaces the old native checkbox + the archive-todo sweep. **"Done" = `Status == "Completed"`**; **"Dropped" = consciously removed/no longer pursuing**. Done-row CF fires on Completed. Completed and Dropped rows stay in place (no relocation), but sort to the bottom for review.
   - **`Horizon`** = native dropdown: `Short Term`, `Long Term`, `Weekly Recurring Sun`..`Weekly Recurring Sat` (extensible later to Quarterly/Yearly). `Long Term`/someday items live here (no separate tab). A **recurring item** = `Horizon` starts with "Weekly Recurring" + `Status == "On-going"`; `build-week` reads these rows directly from `To Do` (NO separate Recurring tab).
...
Rationale: the original single "Live Week" 7-day-pair grid (small text, 7 days competing) drove Kay's overwhelm. The first rebuild over-corrected by deleting the weekly surface entirely; corrected same day to BOTH surfaces. The 2026-05-17 consolidation then collapsed the 4 backend tabs into one Status/Horizon-driven `To Do` tab. Converged via `/socrates` (`brain/outputs/2026-05-17-discussion-daily-tab-tracker-rebuild.md`). Recurring/ongoing items are baseline obligations, not top priorities; when To Do assignments are written into Week, recurring/ongoing rows start below the first three green priority slots even if they also have a Day of the Week value, leaving unused priority slots blank if needed.

**Week-tab layout** (matches the `TO DO 6.7.26` reference): task-only, no habit tracker. Col 0 is label/notes space; for day i (0=Sun..6=Sat) status checkbox col = `1+2*i`, content/task col = `2+2*i`. Row 1 merged A1:O1 title `WEEK OF Jun 7-13` (16pt bold, sage-light) · row 3 `DAILY FOCUS / THEME` · row 6 SUNDAY..SATURDAY 2-col day headers · rows 8–32 twenty-five visible planning slots/day (status checkbox col + task col, wrap, done-row CF), with each day block's first three slots shaded sage · row 34 `notes · ideas · jot`. Native checkboxes, sage palette. Habits live only on day tabs.

**Per-day-tab layout:** row 1 title `SUNDAY · May 17` (20pt bold, sage-dark) · row 4 `HABITS` plus `SUPPLEMENTAL` · rows 5–14 primary habits (A/B checkbox+label), supplemental habits (C/D checkbox+label), and secondary supplemental/goal cells (E/F; Sauna/Glutamine/Ketones/Red light plus GOAL `Protein 120g + Fiber 25g`) · row 16 `DAILY FOCUS` · row 17 focus checkbox + value · row 20 column headers (✓|Task|Type|Project|Notes, 12pt) · rows 21–70 fifty priority slots (A=native checkbox · B=Task **17pt** · C=Type dropdown · D=Project dropdown · E=Notes; ~34px), with rows 21–23 shaded sage as the top-three priority band · row 71 `NOTES` · rows 72–79 free-notes block. **Donut: each day tab DOES retain its native pieChart donut(s) (anchored col G)** and habit completion counts are based on primary habits `A5:A14` / `B5:B14`.

**Week boundary = Sunday** (`today - timedelta(days=(today.weekday()+1)%7)` → Sun..Sat).

**Sunday ceremony** (in `/goodmorning`, NOT `goodnight`): template-first `build-week` is the canonical setup. It creates the new week from the blank template, reconciles prior daily tabs into prior `To Do`, imports the reconciled backend and project tabs, archives prior daily tabs inside the new workbook as far-right history tabs, and places recurring rows on Week as a baseline. Kay then opens the new file, reviews `To Do`, marks known completions/drops, rows sort `On-going` first, `Not Completed` second, and `Completed`/`Dropped` at the bottom, and she schedules active items onto Week. When Kay says the Week tab looks good, `distribute-week` flows Week to daily tabs.

**2026-06-14 correction:** The prior file is no longer the structural source for the new week. The Sunday build uses the blank template as structure, then imports reconciled content. Before import, reconcile prior daily surfaces into prior `To Do`: exact checked-slot completion sync, missing day-tab tasks added to backend, and conservative task-shape cleanup from daily tabs, such as collapsing several stale "Submit the boys to X" backend rows into one combined colon-delimited task when Kay already made that combined edit on a daily tab. Ambiguous/fuzzy completion matches are not auto-written.
...
**Verb changes:** `build-week` creates the new weekly file from the blank template, imports reconciled prior `To Do` and project tabs, archives prior daily tabs far-right, places recurring rows on Week, and validates. Week is the human planning surface; after Kay approval, `distribute-week` flows Week to daily tabs. Week shows the first 25 task slots per day; day tabs retain the full 50 task slots and all habit tracking. `archive` → DEPRECATED alias of `build-week`. **`archive-todo` → RETIRED no-op** (no sweep, no `Completed To Do`; "done" = `Status=Completed` in place; "dropped" = `Status=Dropped`, both sorted to bottom for Sunday review). `recurring-add` / `recurring-remove` write/clear `To Do` rows (Horizon = `Weekly Recurring {day}`, Status `On-going`). `append` gained optional `--horizon` (default `Short Term`). `move-day-item` / `promote` / `schedule-to-day-slot` / `sync-done-status` / `report` / `reformat` operate on the 7 day tabs (slots rows 17–66, habits rows 5–14) and the `To Do` tab.
...
**See the CURRENT ARCHITECTURE "Sunday ceremony" line above.** Triggered by `/goodmorning` on Sunday (NOT `goodnight`). End-to-end: template-first `build-week` creates the new current-week file from the blank template, reconciles prior daily tabs into prior `To Do`, imports reconciled `To Do` and project tabs, archives prior daily tabs far-right in the new workbook, places recurring rows and explicit day assignments on Week, validates, waits for Kay's `To Do`/Week review, then `distribute-week` flows Week to daily tabs after Kay approval.
...
- A **recurring item** = a `To Do` row with `Horizon` = `Weekly Recurring {day}` (e.g. `Weekly Recurring Sun`) and `Status = On-going`.
