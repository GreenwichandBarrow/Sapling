...
**Verb changes:** `build-week` creates the new weekly file from the blank template, imports reconciled prior `To Do` and project tabs, archives prior daily tabs far-right, places recurring rows on Week, and validates. Week is the human planning surface; after Kay approval, `distribute-week` flows Week to daily tabs. Week shows the first 25 task slots per day; day tabs retain the full 25 task slots and all habit tracking. `archive` → DEPRECATED alias of `build-week`. **`archive-todo` → RETIRED no-op** (no sweep, no `Completed To Do`; "done" = `Status=Completed` in place, sorted to bottom for Sunday review). `recurring-add` / `recurring-remove` write/clear `To Do` rows (Horizon = `Weekly Recurring {day}`, Status `On-going`). `append` gained optional `--horizon` (default `Short Term`). `move-day-item` / `promote` / `schedule-to-day-slot` / `sync-done-status` / `report` / `reformat` operate on the 7 day tabs (slots rows 17–41, habits rows 5–14) and the `To Do` tab.
