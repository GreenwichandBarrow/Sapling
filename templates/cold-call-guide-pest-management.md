# G&B Cold Call Guide

**For:** JJ (Jerome)
**Version:** 1.0
**Last Updated:** April 6, 2026

---

## 1. YOUR GOAL

You are calling business owners on behalf of Greenwich & Barrow. Your job is to **connect the owner with Kay** — that's it. You are not selling anything. You are not negotiating. You are opening a door.

A successful call: the owner agrees to a 15-minute introductory call with Kay.

A successful voicemail: the owner has heard our name and knows we'll follow up.

**Do not try to explain the deal structure, valuation, or acquisition process.** If the owner asks detailed questions, say: "That's a great question — Kay would be the best person to walk you through that. Can I get 15 minutes on her calendar for you?"

---

## 2. THE SCRIPT

### Tier 1: You Know the Owner's Name

**Opening:**
> "Hi, is this {Owner Name}?"

*[If yes:]*
> "Hi {Owner Name}, this is JJ calling on behalf of Kay Schneider with Greenwich & Barrow. How are you doing today?"

*[Let them respond — don't rush past this.]*

> "Great — the reason I'm calling is Kay is looking to acquire one {industry} company to own and operate herself, long-term. She's not a private equity firm, not a roll-up — she wants to run one great company. She came across yours and would love to have a quick 15-minute introductory call to learn more about your business. Would you be open to that?"

### Tier 2: You Don't Know the Owner's Name

**Opening:**
> "Hi, may I speak with the owner or the person in charge of the business?"

*[If transferred or owner answers:]*
> "Hi, this is JJ calling on behalf of Kay Schneider with Greenwich & Barrow. And who am I speaking with?"

*[Get and note the name.]*

> "Nice to meet you, {Name}. The reason I'm calling is Kay is looking to acquire one {industry} company to own and operate herself, long-term. She's not a private equity firm, not a roll-up — she wants to run one great company. She came across yours and would love to have a quick 15-minute introductory call to learn more about your business. Would you be open to that?"

### If They Say Yes

> "Great, thank you. Would {suggest two days this week} work? Morning or afternoon?"

Book directly on Kay's calendar if possible. If not:
> "I'll have Kay's office reach out to confirm a time. What's the best email to send that to?"

**Capture:** Owner name, email, preferred time, and any comments they made.

### If They Say "Maybe" or "Send Me Something First"

> "Absolutely — I can have Kay send over a quick intro email. What's the best email address?"

**Capture:** Owner name and email. Flag as warm lead.

### If They Say No

> "No problem at all, I appreciate your time. If anything changes down the road, we'd love to connect. Have a great day."

**Do not push.** Mark as "Not Interested" and move on.

---

## 3. VOICEMAIL SCRIPT

Keep it under 20 seconds. Speak slowly and clearly.

> "Hi {Owner Name}, this is JJ calling on behalf of Kay Schneider with Greenwich & Barrow. Kay is looking to acquire one {industry} company to operate long-term and was interested in learning more about your business. I'll try you again, or you can reach us at barrie@greenwichandbarrow.com. Thanks, have a great day."

**If you don't know the owner's name:**
> "Hi, this is JJ calling on behalf of Kay Schneider with Greenwich & Barrow. Kay is looking to acquire one {industry} company to operate long-term and was interested in learning more about your business. I'll try you again, or you can reach us at barrie@greenwichandbarrow.com. Thanks, have a great day."

**Rules:**
- Leave a voicemail on the FIRST call only. If you call back later and get voicemail again, hang up.
- Speak the email address slowly: "b-a-r-r-i-e at greenwich and barrow dot com"

---

## 4. GATEKEEPER HANDLING

You will talk to receptionists, office managers, and dispatchers more than owners. This is normal.

### Approach: Friendly and Direct

> "Hi, this is JJ with Greenwich & Barrow. I'm trying to reach the owner — is he/she available?"

**If they ask what it's regarding:**
> "Sure — we're a company that acquires and operates businesses in this space, and Kay Schneider, our principal, was hoping to connect with the owner for a brief introductory call."

**If they say the owner is busy/unavailable:**
> "No problem — when's usually a good time to catch them? And may I get their name so I know who to ask for next time?"

**Capture the owner's name.** This is valuable — it upgrades future calls from Tier 2 to Tier 1.

**If they offer to take a message:**
> "That would be great. JJ with Greenwich & Barrow, and the best way to reach us is barrie@greenwichandbarrow.com. We're looking to connect with the owner about a potential business opportunity."

**Do NOT:**
- Say "acquisition" or "buying the business" to a gatekeeper — it can alarm employees
- Leave long messages — keep it simple
- Be rude or impatient — gatekeepers talk to the owner

---

## 5. COMMON OBJECTIONS

### "We're not for sale."
> "Totally understand — most of the owners Kay has spoken with weren't actively looking either. She's really just hoping to introduce herself and learn about your business. No pressure at all. Would a 15-minute call be okay?"

### "Who is Greenwich & Barrow?"
> "Greenwich & Barrow is Kay Schneider's company — she's looking to acquire one business to run full-time as CEO. She's not a private equity firm or a roll-up. She wants to operate one great company long-term."

### "We already have offers / we're working with a broker."
> "Got it — that's great. Kay is always happy to have a conversation even if you're already exploring options. Sometimes it's helpful to see a different approach. Would a quick call make sense?"

### "How did you find us?"
> "We've been researching the industry and your company stood out based on your reputation and track record in the area."

### "What's the offer / what would you pay?"
> "That's a great question and honestly one that Kay would be much better suited to discuss. She'd want to understand your business first before talking numbers. Can I set up a quick call?"

### "Send me an email."
> "Happy to — what's the best email to reach you at?"

**Capture the email. Flag for Kay's follow-up.**

---

## 6. NICHE CONTEXT: Premium Pest Management

You don't need to be a pest control expert. But knowing a few things helps you sound credible.

**The industry:**
- There are 19,000+ pest management companies in the US — it's very fragmented
- The biggest company (Rentokil/Terminix) only has ~12% of the market
- The industry is growing 5-6% per year — faster than most
- Private equity firms have been actively buying pest control companies (60% of M&A activity)

**What owners care about:**
- **Their people** — many owners built the business from scratch and care deeply about their employees
- **Their reputation** — pest control is a trust business, built on years of reliable service
- **Succession** — many founders are 55+ with no family member taking over
- **Competition from roll-ups** — PE firms are consolidating the industry, which worries independent operators

**Why G&B is different from PE:**
- Kay acquires ONE company to run herself — not 10 companies to smash together
- She becomes the CEO and operates day-to-day
- No layoffs, no dramatic changes on Day 1
- The owner's legacy and team are preserved

**Service types you might hear about:**
- General pest (ants, roaches, spiders) — most common
- Termite control — high-value, often separate contracts
- Wildlife/animal removal
- Fumigation
- Commercial food safety / HACCP compliance — this is the premium segment
- Bed bug treatment — growing specialty

---

## 7. LOGISTICS

**Scheduling contact:** barrie@greenwichandbarrow.com (Howie)
**Kay's availability:** Weekdays, flexible — Howie manages her calendar
**Your shift:** Monday-Friday, 10am-2pm ET

**How to log your calls:**
- Use the Daily Call Tab on the target sheet (a new tab is created each morning)
- Fill in: Call Status, Duration, Notes, Owner Sentiment
- If you learn the owner's name, add it to the Notes column — we'll update the master list

**Call Status options:**
- **Connected** — spoke with owner or decision-maker
- **Voicemail** — left a message
- **No Answer** — no pickup, no voicemail
- **Wrong Number** — number doesn't reach the company
- **Gatekeeper** — spoke with receptionist/office staff, didn't reach owner
- **Callback Requested** — they asked you to call back at a specific time

**Owner Sentiment** (only if you spoke with the owner):
- **Interested** — agreed to a call or asked for more info
- **Neutral** — didn't say no, didn't commit
- **Not Interested** — declined

**Important:** If an owner says yes to a call, note that IMMEDIATELY. Don't wait until end of shift.

---

## QUICK REFERENCE CARD

| Situation | Action |
|-----------|--------|
| Owner says yes to call | Book on calendar or get email, flag immediately |
| Owner says "send email first" | Get email, flag as warm lead |
| Owner says no | Thank them, mark Not Interested, move on |
| Gatekeeper won't transfer | Get owner's name, note best time to call back |
| Owner asks detailed deal questions | "Kay would be best to walk you through that" |
| Owner mentions broker or other offers | Note it, still ask for intro call |
| Learn owner's name from gatekeeper/VM | Write it in Notes — free enrichment |
| Voicemail | Leave message on 1st attempt ONLY |
